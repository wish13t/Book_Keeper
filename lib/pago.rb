...
class Pago
	def self.make_payment(order_id:, payment_method:, payment_details:)
			...
			when :credit_card
				Rails.logger.info "Processing credit card: " + payment_details.fetch(:cc_num).to_s + "/" + payment_details.fetch(:expiration_month).to_s + "/" + payment_details.fetch(:expiration_year).to_s
			when :po
				Rails.logger.info "Processing purchase order: " + payment_deatils.fetch(:po_num).to_s
			else
